...
public class Parser {
    ...
    public static Request parsing(String command) {
    ...
    private static Request incorrectCommand() { return Request.makeIncorrect();}

    private static Request firstIsViewOrRemove(ArrayList<Key> keys, ArrayList<String> arguments) {
        ...
        switch (keys.get(1)) {
            ...
            case TRACK:
                ...
                return arg.equals("all") || id >= 0 || keys.get(1) == Key.GENRE ?
                        new Request(true, keys, arguments)
                        : incorrectCommand();
            default:
                return incorrectCommand();
        }
    ...
    private static Request firstIsAdd(ArrayList<Key> keys, ArrayList<String> arguments) {
        if(keys.size() != 2)
            return incorrectCommand();
        switch (keys.get(1)) {
            case GENRE:
    ...
    private static Request firstIsEdit(ArrayList<Key> keys, ArrayList<String> arguments) {
        ...
        switch (keys.get(1)) {
            case GENRE:
                if (keys.get(2) != Key.NAME)
                    return incorrectCommand();
                return new Request(true, keys, arguments);
    ...
    private static Request firstIsFind(ArrayList<Key> keys, ArrayList<String> arguments) {
    ...
    private static Request firstIsSave(ArrayList<Key> keys, ArrayList<String> arguments) {
        if (keys.size() > 2 || arguments.size() != 1)
            return incorrectCommand();
    ...
    private static Request firstIsLoad(ArrayList<Key> keys, ArrayList<String> arguments) {
        if (keys.size() != 2 || arguments.size() != 1)
            return incorrectCommand();
        switch (keys.get(1)) {
            case OVERWRITE:
    ...
    private static Request parsedCommand(ArrayList<Key> keys, ArrayList<String> arguments) {
        switch (keys.get(0)) {
            case GET:
            case REMOVE:
    ...
    private static Request withoutArgs(String[] command) {
